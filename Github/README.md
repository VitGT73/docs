...
Если хотим сохранять историю о том в каких ветках производились изменения
...
git branch
```
Если не в основной, переходим в неё
```bash
git checkout main
```
Обновите локальную копию 'main' из репозитория GitHub:
```bash
git pull origin main
```
Слейте ветку 'my/branch' в 'main' с использованием --no-ff:
```bash
git merge my/branch
```
Зафиксируйте изменения:
```bash
git commit -m "Merge branch 'my/branch' into main"
```
...
git branch -d my/branch # удаляем поглощаемую ветку (безопасный вариант - удаляет ветку только, если все коммиты добавлены в main )
```

```bash
git branch -D my/branch # удаляет поглощаемую ветку в любом случае
```
 Удаление локальной ветки через Git не удаляет ветку на GitHub! Для ее удаления:
```bash
git push origin --delete install
```
...
Если вы уже участник проекта (или collaborator в терминах GitHub), можно клонировать репозиторий напрямую. А если нет, нужно предварительно сделать «форк». Также для участников доступна кнопка Merge после ревью, а для не участников — нет.
